...
/**
 * Tab per la gestione delle Location Stripe Terminal associate all'evento.
 * Mostra le location già associate (con reader), permette di aggiungere reader
 * da location esistenti e di creare nuove location.
 *
 * @param {Object} props
 * @param {number} props.eventId - ID dell'evento
 */
export function EventLocations({ eventId }) {
  const [eventLocations, setEventLocations] = useState([]);
  const [allLocations, setAllLocations] = useState([]);
  const [loading, setLoading] = useState(true);

  // Stato per "Aggiungi reader esistente"
  const [selectedLocationId, setSelectedLocationId] = useState("");
  const [selectedReaderId, setSelectedReaderId] = useState("");
  const [adding, setAdding] = useState(false);

  // Stato per "Crea nuova location"
  const [showNewLocationForm, setShowNewLocationForm] = useState(false);
  const [newLocationForm, setNewLocationForm] = useState(EMPTY_NEW_LOCATION_FORM);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (eventLocRes.ok) {
        const data = await eventLocRes.json();
        setEventLocations(data.data.locations ?? []);
      }
      if (allLocRes.ok) {
        const data = await allLocRes.json();
  ...
  // Calcola i reader già associati all'evento (per escluderli dalla select)
  const associatedReaderIds = new Set(
    eventLocations.flatMap((loc) => (loc.readers ?? []).map((r) => r.id))
  );

  // Reader disponibili nella location selezionata (esclusi quelli già associati)
  const selectedLocation = allLocations.find(
    (loc) => loc.id === Number(selectedLocationId)
  );
  const availableReaders = (selectedLocation?.readers ?? []).filter(
    (r) => !associatedReaderIds.has(r.id)
  );
  ...
  const handleRemoveReader = async (readerId) => {
    try {
      const res = await apiRequest({
  ...
  const handleNewLocationFormChange = (e) => {
  ...
  return (
    <div>
              ...
              {eventLocations.map((loc) => (
                ...
                >
                  ...
                  {(loc.readers ?? []).length === 0 ? (
                    <p className="text-gray-400 text-xs italic ml-5">
      ...
      {/* Sezione: Aggiungi POS da location esistente */}
      <div className="shadow-sm rounded-lg bg-white mb-4">
        <div className="p-4">
          <h6 className="font-semibold text-sm text-gray-700 mb-3">
            Aggiungi POS da location esistente
          </h6>
