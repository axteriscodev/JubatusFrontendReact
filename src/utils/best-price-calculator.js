...
export function calculatePrice(packages, reqP, reqV, reqC) {
  ...
  const dp = Array.from({ length: reqP + 1 }, () =>
    Array.from({ length: reqV + 1 }, () =>
      Array(reqC + 1).fill(Infinity)
    )
  );
  ...
  const normalized = packages.map(pkg => ({
    ...
    r: pkg.quantityClip === -1 ? reqC : pkg.quantityClip,
    price: pkg.price
  }));
  ...
  for (let p = 0; p <= reqP; p++) {
    for (let v = 0; v <= reqV; v++) {
      for (let r = 0; r <= reqC; r++) {
        if (dp[p][v][r] === Infinity) continue;

        for (const pkg of normalized) {
          const nP = Math.min(p + pkg.p, reqP);
          const nV = Math.min(v + pkg.v, reqV);
          const nR = Math.min(r + pkg.r, reqC);

          if (dp[p][v][r] + pkg.price < dp[nP][nV][nR]) {
            dp[nP][nV][nR] = dp[p][v][r] + pkg.price;
          }
        }
      }
    }
